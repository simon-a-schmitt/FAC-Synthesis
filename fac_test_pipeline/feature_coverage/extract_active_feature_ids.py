...
INPUT_TSV = os.path.join(
    _SCRIPT_DIR, "output", "bbfg_deepseek_05_fc_content.tsv"
)
THRESHOLD = 1.5
OUTPUT_JSON = os.path.join(
    _SCRIPT_DIR, "output", "bbfg_deepseek_05_fc_content_active_feature_ids.json"
)
...
def main() -> None:
    ...
    with open(INPUT_TSV, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            if float(row["max_normalized_magnitude"]) > THRESHOLD:
    ...
    print(
        f"{len(active_feature_ids)} features with max_normalized_magnitude > "
