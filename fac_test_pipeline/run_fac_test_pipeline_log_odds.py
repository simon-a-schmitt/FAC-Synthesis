...
def load_baseline_full(tsv_path: str) -> Dict[int, Dict[str, float]]:
    """Load p95, activation_count, and total_tokens per feature from the baseline TSV."""
...
def _compute_log_odds_se(a_seed: float, N_seed: float, a_base: float, N_base: float) -> float:
    """Smoothed log-odds ratio divided by its Woolf standard error."""
    lor = (
...
def extract_token_feature_matrix_for_prompt(
    ...
    collector: Collector,
    sae: TopKSAE,
    baseline_full: Optional[Dict[int, Dict[str, float]]] = None,
) -> Dict[str, Any]:
    ...
    # Derive p95 lookup for normalization from the full baseline data.
    baseline_p95: Optional[Dict[int, float]] = None
    if baseline_full is not None:
        baseline_p95 = {fid: v["p95"] for fid, v in baseline_full.items()}
    ...
    # Build sub-matrix, normalize by p95, and zero out values <= 0.6.
    ...
    # --- Log-odds / SE computation ---
    # a_seed: raw activation counts per feature on content tokens (no 0.6 filter).
    # a_base, N_base: from the baseline TSV.
    log_odds_se_list: List[float] = []
    if baseline_full is not None and rel_feature_tensor.numel() > 0:
        # Count raw (unfiltered) activations per surviving feature across content tokens.
        raw_counts = (content_features[:, rel_feature_tensor] > 0).sum(dim=0)  # [n_rel_features]
        for i, fid in enumerate(rel_feature_tensor.tolist()):
            fid_int = int(fid)
            a_seed = float(raw_counts[i].item())
            bdata = baseline_full[fid_int]
            metric = _compute_log_odds_se(
                a_seed=a_seed,
                N_seed=float(N_seed),
                a_base=bdata["activation_count"],
                N_base=bdata["total_tokens"],
            )
            log_odds_se_list.append(round(metric, 4))
    ...
    return {
        ...
        "log_odds_se": log_odds_se_list,
        "matrix_shape": [int(sparse_features.shape[0]), int(rel_feature_tensor.numel())],
        "feature_matrix": feature_matrix,
        "excluded_features": excluded_features,
        "normalized": baseline_full is not None,
        "n_seed_tokens": N_seed,
    }
...
<style>
  ...
  .search-input { padding: 7px 12px; border-radius: 8px; border: 1px solid var(--border-strong);
    background: var(--bg-secondary); color: var(--text-primary); font-size: 13px; outline: none; width: 220px; }
  .search-input:focus { border-color: var(--accent); }
  .feat-count-badge { font-size: 12px; color: var(--text-secondary); white-space: nowrap; }
  /* Sort buttons */
  .sort-buttons { display: flex; gap: 4px; }
  .sort-btn { padding: 5px 10px; border-radius: 6px; border: 1px solid var(--border-strong);
    background: var(--bg-secondary); color: var(--text-secondary); font-size: 12px;
    cursor: pointer; white-space: nowrap; }
  .sort-btn.active { background: var(--accent); color: #fff; border-color: var(--accent); }
  .sort-btn.active.lo { background: var(--accent2); border-color: var(--accent2); }
  /* Feature list */
  ...
  .feat-bar-wrap { width: 64px; height: 5px; background: var(--bg-secondary);
    border-radius: 3px; overflow: hidden; flex-shrink: 0; }
  .feat-bar { height: 100%; border-radius: 3px; }
  .feat-bar.mag { background: var(--accent); }
  .feat-bar.lo  { background: var(--accent2); }
  .feat-mag-val { font-size: 11px; min-width: 38px; text-align: right;
    font-variant-numeric: tabular-nums; color: var(--text-secondary); flex-shrink: 0; }
  .feat-lo-val  { font-size: 11px; min-width: 46px; text-align: right;
    font-variant-numeric: tabular-nums; color: var(--accent2); flex-shrink: 0; }
...
<div class="container">
  ...
  <div class="explorer-header">
    ...
    <input class="search-input" id="featSearch" type="text"
      placeholder="Feature-ID oder Beschreibung&hellip;" />
    <div class="sort-buttons">
      <button class="sort-btn active" id="sortMagBtn" onclick="setSort('magnitude')">Magnitude</button>
      <button class="sort-btn lo" id="sortLOBtn" onclick="setSort('log_odds')">Log-Odds / SE</button>
    </div>
...
const LOG_ODDS_SE = __LOG_ODDS_SE__;
...
const featStats = REL_FEATURES.map((fid, j) => {
  ...
  const logOddsSe = LOG_ODDS_SE[String(fid)] !== undefined ? LOG_ODDS_SE[String(fid)] : null;
  return {fid: fid, j: j, maxVal: maxVal, activations: activations, spans: spans, preview: preview,
          logOddsSe: logOddsSe};
});

const maxLogOddsSe = Math.max(0, ...featStats.map(f => f.logOddsSe !== null ? f.logOddsSe : 0));
...
function setSort(mode) {
  sortMode = mode;
  document.getElementById('sortMagBtn').classList.toggle('active', mode === 'magnitude');
  document.getElementById('sortLOBtn').classList.toggle('active', mode === 'log_odds');
  renderList();
}
...
function renderList() {
  ...
  if (sortMode === 'log_odds') {
    sorted.sort((a, b) => (b.logOddsSe !== null ? b.logOddsSe : -Infinity) -
                          (a.logOddsSe !== null ? a.logOddsSe : -Infinity));
  } else {
    sorted.sort((a, b) => b.maxVal - a.maxVal);
  }
  ...
  sorted.forEach(fs => {
    ...
    // Bar percentage depends on active sort mode.
    let pct = 0;
    let barClass = 'mag';
    if (sortMode === 'log_odds') {
      barClass = 'lo';
      pct = maxLogOddsSe > 0 && fs.logOddsSe !== null
        ? Math.max(0, fs.logOddsSe / maxLogOddsSe * 100).toFixed(1)
        : 0;
    } else {
      pct = MAX_VAL > 0 ? (fs.maxVal / MAX_VAL * 100).toFixed(1) : 0;
    }

    const magStr = fs.maxVal.toFixed(2);
    const loStr  = fs.logOddsSe !== null ? fs.logOddsSe.toFixed(2) : 'n/a';
    ...
    hdr.innerHTML =
      ...
      '<div class="feat-bar-wrap"><div class="feat-bar ' + barClass + '" style="width:' + pct + '%;"></div></div>' +
      '<span class="feat-mag-val" title="Max. normalisierte Magnitude">' + magStr + '</span>' +
      '<span class="feat-lo-val" title="Log-Odds / SE">' + loStr + '</span>' +
...
def generate_heatmap_html(
    ...
    log_odds_se_raw: List[float] = record.get("log_odds_se", [])
    ...
    # Build {str(fid): log_odds_se_value} dict for embedding.
    log_odds_se_json: Dict[str, float] = {}
    for fid, lo_se in zip(rel_feats, log_odds_se_raw):
        log_odds_se_json[str(fid)] = lo_se
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="FAC log-odds pipeline: prompt -> token-level SAE features ranked by magnitude and log-odds/SE."
    )
    ...
    parser.add_argument(
        ...
        help="Path to feature_activation_baseline_agg.tsv for p95 normalization and log-odds baseline. "
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to a TSV file with columns FeatureID and Words. "
             "Only rows for active features are loaded. Optional.",
    )
    ...
    with tc.no_grad():
        for idx, prompt in enumerate(tqdm.tqdm(prompts, desc="Processing prompts")):
            record = extract_token_feature_matrix_for_prompt(
                prompt, model, collector, sae, baseline_full=baseline_full
            )
            record["index"] = idx
            ...
            if args.emit_html:
                ...
                title = f"SAE Feature Explorer (Log-Odds) · Prompt {idx} · Layer {record['layer']}"
