"""
Greedily select a subset of pool prompts that together touch as *few
distinct* SAE features as possible, then extract the corresponding records
from the original pool input file.

This is the mirror image of run_feature_coverage_filter_select.py: instead
of picking prompts that maximise the number of distinct active features
covered, it picks prompts that keep that number as low as possible.
...
1. Per prompt, only features with peak_magnitude >= --threshold (default
   1.5) count as "active".
2. Greedy minimum-coverage selection: repeatedly pick the not-yet-selected
   prompt that introduces the fewest features not already touched by the
   prompts selected so far (prompts with zero active features are picked
   first). Ties are broken by the smaller pool_id. Repeat until --n prompts
   are chosen (or the pool is exhausted). This directly minimises the
   number of distinct active features in the selection; --cap only affects
   the capped-activation figures printed for reporting (matching the
   max-coverage script's output), not the selection itself.
...
# ---------------------------------------------------------------------------
# Greedy minimum-coverage selection with a per-feature multiplicity cap
# ---------------------------------------------------------------------------

def greedy_select_prompts_min(
    active_features: Dict[Any, Set[int]],
    n: int,
    cap: int = 3,
) -> Tuple[List[Any], Dict[int, int], Dict[int, int]]:
    """Greedily pick up to n pool_ids minimising the number of *distinct*
    active features touched by the selection.

    At each step the prompt that introduces the fewest features not yet
    seen among the already-selected prompts is chosen (prompts with no
    active features are picked first, since their gain is 0). Ties in gain
    are broken by the smaller pool_id. `cap` does not influence this
    decision — it only affects the capped_counts returned for reporting,
    exactly as in the max-coverage script.
    ...
    for _ in range(n_steps):
        ...
        for pid in sorted(remaining):
            ...
            gain = sum(1 for fid in feats if fid not in seen_features)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Greedily select n pool prompts minimising SAE-feature coverage "
    ...
    parser.add_argument(
        ...
        "feature_coverage_filter/output/<input-json-stem>_greedymin_top<n>_t<threshold>.json.",
    )
...
def main() -> None:
    ...
    selected_pool_ids, capped_counts, raw_counts = greedy_select_prompts_min(
    ...
    if not output_json:
        ...
        output_json = os.path.join(
            _SCRIPT_DIR, "output", f"{stem}_greedymin_top{args.n}_t{threshold_str}.json"
        )
