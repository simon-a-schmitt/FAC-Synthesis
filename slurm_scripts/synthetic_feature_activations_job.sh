...
INPUT_JSONL="$WS_PATH/code/FAC-Synthesis/fac_test_pipeline/input/prompts_seed_casehold.json"
OUTPUT_JSONL="$WS_PATH/code/FAC-Synthesis/fac_test_pipeline/output/feature_matrix_seed_casehold"

mkdir -p logs "$(dirname "$OUTPUT_JSONL")"

# Resumable: re-running this job continues from the examples not yet present
# in $OUTPUT_JSONL, so it is safe to resubmit after hitting the time limit.
python "$WS_PATH/code/FAC-Synthesis/fac_test_pipeline/run_fac_test_pipeline_feature_matrix.py" \
    --prompts-json "$INPUT_JSONL" \
