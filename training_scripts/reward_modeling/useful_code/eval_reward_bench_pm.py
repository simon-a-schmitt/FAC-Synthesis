...
model = AutoModelForCausalLM.from_pretrained(script_args.reward_name_or_path, token=token,
                                             torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2").cuda()
tokenizer = AutoTokenizer.from_pretrained(script_args.reward_name_or_path, token=token, use_fast=True)
tokenizer_plain = AutoTokenizer.from_pretrained(script_args.reward_name_or_path, token=token, use_fast=True)
