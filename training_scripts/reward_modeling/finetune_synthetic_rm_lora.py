...
def build_dataset(tokenizer, train_path):
    ...
    split = ds.train_test_split(test_size=0.01)
...
trainer = RewardTrainer(
    ...
    callbacks=[EvalRewardBenchCallback(eval_interval=args.eval_every_steps, tokenizer=tokenizer, model_name=args.train_set_path, seed=args.seed)],
)
...
print("\n RewardBench ...")
m = model.module if hasattr(model, "module") else model
record_path = f"./logs_base/rewardbench/{args.train_set_path.replace('/', '_')}/seed{args.seed}/final_eval.txt"
os.makedirs(os.path.dirname(record_path), exist_ok=True)
res = run_rewardbench_eval(
    model_or_path=m,
    tokenizer=tokenizer,
